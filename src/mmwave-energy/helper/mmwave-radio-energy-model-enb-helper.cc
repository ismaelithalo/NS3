...
MmWaveRadioEnergyModelEnbHelper::DoInstall(Ptr<NetDevice> device, Ptr<EnergySource> source) const
{
    ...
    Ptr<MmWaveRadioEnergyModelEnb> model =
        m_radioEnergy.Create()->GetObject<MmWaveRadioEnergyModelEnb>();
    NS_ASSERT(model != NULL);
    model->SetNode(node);
    Ptr<LteEnbNetDevice> mmwaveEnbDevice;
    Ptr<LteEnbPhy> mmwavePhy;

    if (deviceName == "ns3::LteEnbNetDevice")
    {
        mmwaveEnbDevice = DynamicCast<LteEnbNetDevice>(device);
    ...
    mmwaveDlSpectrumPhy->TraceConnectWithoutContext(
        "State",
        MakeCallback(&MmWaveRadioEnergyModelEnb::ChangeStateEvent, model));
    mmwaveUlSpectrumPhy->TraceConnectWithoutContext(
        "State",
        MakeCallback(&MmWaveRadioEnergyModelEnb::ChangeStateEvent, model));
