...
MmWaveRadioEnergyModelHelper::DoInstall(Ptr<NetDevice> device, Ptr<EnergySource> source) const
{
    ...
    model->SetNode(node);
    Ptr<LteUeNetDevice> mmwaveDevice;
    Ptr<LteUePhy> mmwavePhy;

    if (deviceName == "ns3::LteUeNetDevice")
    {
        mmwaveDevice = DynamicCast<LteUeNetDevice>(device);
    ...
    mmwaveDlSpectrumPhy->TraceConnectWithoutContext(
        "State",
        MakeCallback(&MmWaveRadioEnergyModel::ChangeStateEvent, model));
